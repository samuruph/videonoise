...
All CLI argument definitions live here — step scripts never define their own
argparse for config params. To add a new config field:
  1. Add it to config.yaml
  2. Add it to the Config dataclass below
  3. Add it to build_parser() with the right type
  4. Apply the override in _apply_overrides()
That's it — no other file needs to change.

Usage
-----
In a step script with no step-specific args:

    from scripts.config_loader import load_config
    cfg = load_config()          # reads config.yaml + any CLI overrides

In a step script that adds its own args:

    from scripts.config_loader import build_parser, load_config
    parser = build_parser("My step description")
    parser.add_argument("--my_flag", ...)
    args = parser.parse_args()
    cfg  = load_config(args)     # merges YAML + args
"""
...
def build_parser(description: str = "") -> argparse.ArgumentParser:
    ...
    All args default to None — only the ones explicitly passed override YAML.
    ...
    p.add_argument("--resize",     type=int, nargs=2, help="Resize frames: W H",
                   metavar=("W", "H"))
...
def _apply_overrides(c: dict, args: argparse.Namespace) -> None:
    """Copy non-None CLI args into the raw config dict (YAML values as fallback)."""
    mapping = {
        "data_real":    "data_real",
        "data_gen":     "data_gen",
        "gen_dir":      "gen_dir",
...
def load_config(args: Optional[argparse.Namespace] = None) -> Config:
    ...
    Pass a pre-parsed Namespace when the calling script added its own arguments.
    """
    if args is None:
    ...
    return Config(
        ...
        resize        = tuple(resize),
        model         = c["model"],
        model_id      = c.get("model_id") or None,
        t2i_model_id  = c.get("t2i_model_id") or None,
